...
async def received_subjects(update: Update, context: CallbackContext) -> int:
    ...
    await update.message.reply_text(
        "آزمون یکی از مهم‌ترین ابزارها برای برنامه‌ریزی درسی است. به نظر شما هدف از شرکت در آزمون‌های قلم‌چی چیست؟",
        reply_markup=reply_markup
    )
    return CHOICE_GOAL
...
async def received_goal(update: Update, context: CallbackContext) -> int:
    ...
    await update.message.reply_text("برای دریافت برنامه مطالعاتی رایگان و مشاوره با رتبه‌های برتر ، شماره تماس خود را وارد کنید:", reply_markup=reply_markup)
    return CHOICE_PHONE
...
async def received_phone(update: Update, context: CallbackContext) -> int:
    ...
    if not is_valid_phone(phone):
        await update.message.reply_text("شماره تلفن وارد شده معتبر نیست. لطفاً با فرمت صحیح وارد کنید (مثلاً: 09123456789).")
        return CHOICE_PHONE
...
def main():
    application = ApplicationBuilder().token("7949641234:AAEBsEKqGwwzDdzPxA2c4Wwt_Gf-XFbTWVE").build()
    ...
    conv_handler = ConversationHandler(
        ...
        states={
            ...
            CHOICE_PHONE: [MessageHandler(filters.ALL, received_phone)],
